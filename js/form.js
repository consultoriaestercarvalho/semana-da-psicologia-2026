...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      const protocolo = gerarProtocolo(dados);
      dados.protocolo = protocolo;

      await enviarParaPlanilha(dados);

      // Guarda os dados localmente para as próximas páginas
      // (pagamento.html e confirmacao.html).
      sessionStorage.setItem("inscricao_dados", JSON.stringify(dados));
      sessionStorage.setItem("inscricao_protocolo", protocolo);
  ...
  function redirecionarParaPagamento() {
    // Leva para a página com o Pix (copia-e-cola + QR Code) e o
